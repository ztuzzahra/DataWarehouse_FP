<?php
	...
	$kategori = $conn->query("SELECT CONCAT('name:',f.judul) as name, CONCAT('y:', SUM(fp.amount)*100/" . $tot_amount[0] .") as y, CONCAT('drilldown:', f.judul) as drilldown 
		FROM film f 
	...
	$hasil_kat = $conn->query("SELECT f.judul judul, sum(fp.amount) as tot_kat
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Presentase Nilai Penjualan (WH Sakila) - Semua Judul Film'
    },
    subtitle: {
        text: 'Klik bar untuk melihat detil nilai penjualan Judul Film berdasarkan bulan'
    },
   accessibility: {
		announceNewData: {
			enabled: true
