<?php
	...
	//query untuk ambil penjualan berdasarkan TAHUN
	//query teknik gak jelas

	$kategori = $conn->query("SELECT CONCAT('name:',t.tahun) as name, CONCAT('y:', SUM(fp.amount)*100/" . $tot_amount[0] .") as y, CONCAT('drilldown:', t.tahun) as drilldown 
		FROM time t 
	...
	$hasil_kat = $conn->query("SELECT t.tahun tahun, sum(fp.amount) as tot_kat
	...
	//query untuk ambil penjualan di nama hari berdasarkan tahun (clean)
	$det_kat = $conn->query("SELECT t.tahun tahun,
			t.namahari as namahari,
			sum(fp.amount) as pendapatan_kat
			FROM time t
			JOIN fakta_pendapatan fp ON (t.time_id = fp.time_id)
			GROUP BY tahun, namahari");
	$i = 0;
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Presentase Nilai Penjualan (WH Sakila) - Semua Tahun'
    },
    subtitle: {
        text: 'Klik bar untuk melihat detil nilai penjualan pertahun berdasarkan Nama Hari'
    },
   accessibility: {
		announceNewData: {
			enabled: true
