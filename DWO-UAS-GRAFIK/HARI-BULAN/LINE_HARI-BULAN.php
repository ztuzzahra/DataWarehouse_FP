<?php
	...
	//query untuk ambil penjualan berdasarkan nama hari
	//query teknik gak jelas

	$kategori = $conn->query("SELECT CONCAT('name:',t.namahari) as name, CONCAT('y:', SUM(fp.amount)*100/" . $tot_amount[0] .") as y, CONCAT('drilldown:', t.namahari) as drilldown 
		FROM time t 
	...
	//query untuk tahu SUM(Amount) semua nama hari
	$hasil_kat = $conn->query("SELECT t.namahari namahari, sum(fp.amount) as tot_kat
	...
	//query untuk ambil penjualan di nama hari berdasarkan bulan (clean)
	$det_kat = $conn->query("SELECT t.namahari namahari,
			t.bulan as bulan,
			sum(fp.amount) as pendapatan_kat
			FROM time t
			JOIN fakta_pendapatan fp ON (t.time_id = fp.time_id)
			GROUP BY namahari, bulan");
	$i = 0;
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Presentase Nilai Penjualan (WH Sakila) - Semua Hari'
    },
    subtitle: {
        text: 'Klik bar untuk melihat detil nilai penjualan perhari berdasarkan bulan'
    },
   accessibility: {
		announceNewData: {
			enabled: true
