<?php
	...
	$kategori = $conn->query("SELECT CONCAT('name:',f.kategori) as name, CONCAT('y:', SUM(fp.amount)*100/" . $tot_amount[0] .") as y, CONCAT('drilldown:', f.kategori) as drilldown 
		FROM film f 
	...
	$hasil_kat = $conn->query("SELECT f.kategori kategori, sum(fp.amount) as tot_kat
...
Highcharts.chart('container', {
    ...
    subtitle: {
        text: 'Klik bar untuk melihat detil nilai penjualan perkategori berdasarkan Nama Hari'
    },
   accessibility: {
		announceNewData: {
			enabled: true
