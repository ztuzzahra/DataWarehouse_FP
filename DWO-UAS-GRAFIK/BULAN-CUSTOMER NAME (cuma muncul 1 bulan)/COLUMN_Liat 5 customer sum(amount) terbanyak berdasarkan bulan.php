<?php
	...
	//query untuk ambil penjualan per bulan berdasarkan nama customer (clean)
	$det_kat = $conn->query("SELECT t.bulan bulan,
			c.nama as nama,
			sum(fp.amount) as pendapatan_kat
			FROM time t
			JOIN fakta_pendapatan fp ON (t.time_id = fp.time_id)
			JOIN customer c ON (c.customer_id = fp.customer_id)
			GROUP BY bulan, nama
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Presentase Nilai Penjualan (WH Sakila) - Semua Customer'
    },
    subtitle: {
        text: 'Klik bar untuk melihat detil nilai penjualan customer berdasarkan bulan'
    },
   accessibility: {
		announceNewData: {
			enabled: true
	...
	series: [
		{
			name: "Pendapatan By Customer Name",
			colorByPoint: true,
			data: 
				<?php
					//TEKNIK GAK JELAS :D
